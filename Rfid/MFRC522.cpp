...
void MFRC522::PCD_ReadRegister(	PCD_Register reg,		///< The register to read from. One of the PCD_Register enums.
	...
	byte address = 0x80 | reg;				// MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.						// Index in values array.
	digitalWrite(_chipSelectPin, LOW);		// Select slave
	...
	values[count] = SPI.transfer(0);			// Read the final byte. Send 0 to stop reading.
	digitalWrite(_chipSelectPin, HIGH);			// Release slave again
} // End PCD_ReadRegister()
...
bool MFRC522::PCD_PerformSelfTest() {
	...
	assertF("Version: "); assert2ln(version, HEX);
	assertln();
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte MFRC522::PCD_CommunicateWithPICC(	PCD_Command command,		///< The command to execute. One of the PCD_Command enums.
	...
	if (backData && backLen) {
		...
		*backLen = n;											// Number of bytes returned
		byte value = backData[0];
		PCD_ReadRegister(FIFODataReg, n, backData);				// Get received data from FIFO

		if (rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
			// Create bit mask for bit positions rxAlign..7
			byte mask = 0;
			for (byte i = rxAlign; i < 8; i++) {
				mask |= _BV(i);
			}
			// Apply mask to both current value of values[0] and the new data in value.
			backData[0] = (value & ~mask) | (backData[0] & mask);
		}
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
byte MFRC522::PICC_REQA_or_WUPA(	PICC_Command command, 		///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
	...
	status = PCD_TransceiveData((byte*)&command, 1, bufferATQA, bufferSize, &validBits);
	if (status != STATUS_OK) {
		return status;
	}
 ...
 */
byte MFRC522::PCD_Authenticate(PICC_Command command,		///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
