...
public:
               ...
               _i(i), _debounce(debounce)
   {
      ...
      for (uint8_t i = 0; i < _i; i++) {
         ...
         if (*_watchState[i]) { _oldState[i >> 3] |= _BV(i); }
      }
   ...
   uint8_t* operator() (void)
   {
      uint32_t tempTime = millis();
      for (uint8_t i = 0; i < _i; i++) {
         if ( *_watchState[i] != (bool)(_oldState[i >> 3] & _BV(i)) ) {
            if ((tempTime - _saveTime[i]) >= _debounce) {
               _saveTime[i] = tempTime;
               *_watchState[i] ? _oldState[i >> 3] |= _BV(i) : _oldState[i >> 3] &= ~_BV(i);
            }
         }
      }
...
public:
               ...
               _i(i), _debounce(debounce)
   {
      ...
      for (uint8_t i = 0; i < _i; i++) {
         ...
         if (bTemp) { _oldState[i >> 3] |= _BV(i); _dOldState[i >> 3] |= _BV(i); }
      }
   ...
   void loop()
   {
      ...
      for (uint8_t i = 0; i < _i; i++) {
         bool bTemp = digitalRead(_watchState[i]);
         if (INVERTED) { bTemp = !bTemp; }
         bTemp ? _oldState[i >> 3] |= _BV(i) : _oldState[i >> 3] &= ~_BV(i);

         if ( (_oldState[i >> 3] & _BV(i)) != (_dOldState[i >> 3] & _BV(i)) ) {
            if ((tempTime - _saveTime[i]) >= _debounce) {
               _saveTime[i] = tempTime;
               (_oldState[i >> 3] & _BV(i)) ? _dOldState[i >> 3] |= _BV(i) : _dOldState[i >> 3] &= ~_BV(i);
            }
         }
      }
   }
