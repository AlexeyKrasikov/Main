...
public:
   // В качестве параметров с переменным числом аргументов передаются адреса отслеживаемых булевых переменных
   dStructArrayBool(uint8_t j, uint8_t debounce) :    
               _i(j), _debounce(debounce), _watchState(0), _num_byte(_i / 8)
   {
      _saveTime = new uint32_t[_i]; 
      uint8_t* pNumByte = (uint8_t*)&_num_byte;
      if ((_i & 0b111) != 0) { (*pNumByte)++; }     
      _oldState = new uint8_t[_num_byte];
   }

   dStructArrayBool(uint8_t j, uint8_t debounce, bool* watchState) :    
               _i(j), _debounce(debounce), _num_byte(_i / 8)
   {
      dStructArrayBool(j, debounce);
   ...
   void initOldState(bool* watchState)
   {
      ...
      for (uint8_t i = 0; i < _num_byte; i ++) { _oldState[i] = 0; }
   ...
   const uint8_t* getDState() { return _oldState; }
   uint8_t getByte() { return (uint8_t)_num_byte; }
